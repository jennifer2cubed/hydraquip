				...
				<a class="product-content-image" href="<?php the_permalink(); ?>" data-images="<?php echo get_images_list(); ?>">
					...
					<?php
						if ( has_post_thumbnail() ) {
							...
							echo sprintf('<img data-src="%s" class="owl-lazy" alt="%s" title="%s" data-srcset="%s" sizes="%s" height="%s" width="%s" />',esc_url( $thumbnail_image[0] ),esc_attr($props['alt']),get_the_title(),wp_get_attachment_image_srcset( get_post_thumbnail_id(), apply_filters( 'single_product_archive_thumbnail_size', 'shop_catalog' )), wp_get_attachment_image_sizes( get_post_thumbnail_id(), apply_filters( 'single_product_archive_thumbnail_size', 'shop_catalog' )), esc_attr($thumbnail_image[1]), esc_attr($thumbnail_image[2]) );

						} elseif ( wc_placeholder_img_src() ) {
							echo wc_placeholder_img( apply_filters( 'single_product_archive_thumbnail_size', 'shop_catalog' ) );
						}
					 ?>
                    <?php if($hover == 'mask'): ?>
