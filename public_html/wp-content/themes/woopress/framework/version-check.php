...
class ETheme_Version_Check {
    ...
    public function activation_page() {
        $this->process_form();
        ?>
            <h1><?php esc_html_e( 'Activate Theme', 'woopress' ); ?></h1>
            <?php if ( etheme_is_activated() ): ?>
                <div class="etheme-options-success">
                    <p><strong><?php esc_html_e('Thank you for activation', 'woopress'); ?></strong></p>
            ...
            <?php else: ?>
                <p><?php esc_html_e('Use your purchase code to activate etheme template. Please, note, that you won’t be able to use it without activation.', 'woopress'); ?></p>
                <p><?php esc_html_e('To find your Purchase code, please, enter your ThemeForest account > Downloads tab > choose etheme > Download > License Certificate & Purchase code', 'woopress'); ?> <a href="http://prntscr.com/d23p2c" target="_blank">http://prntscr.com/d23p2c</a></p>
                <p><?php esc_html_e('Activate etheme template and get lifetime updates, 6 months of free top-notch support, 24/7 live support and much more.', 'woopress'); ?></p>
                <p><?php esc_html_e('A purchase code (license) is only valid for One Project. Do you want to use this theme for a one more project? Purchase a ', 'theme'); ?> <a href="https://themeforest.net/item/xstore-responsive-woocommerce-theme/8611976?license=regular&open_purchase_for_item_id=8611976&purchasable=source&ref=8theme"><?php esc_html_e('new license here', 'etheme'); ?></a><?php esc_html_e(' to get a new purchase code.', 'etheme'); ?></p>
    ...
    public function process_form() {
        if( isset( $_POST['etheme-purchase-code'] ) && ! empty( $_POST['etheme-purchase-code'] ) ) {
            ...
            if( empty( $code ) ) {
                echo  '<p class="error">' . esc_html__('Enter the purchase code', 'woopress') . '</p>';
                return;
            }
            $theme_id = 9751050;
            ...
            if( $response_code != '200' ) {
                echo   '<p class="error">' . esc_html__('API request call error.', 'woopress') . "<br/>" . esc_html__('1) Contact your server provider and make sure that OpenSSL system library is 1.0 or higher', 'woopress') . "<br/>" . esc_html__('2) Make sure that your purshase code is associated with buyer. If you bought theme using guest account create account with the same email that was used during purchase process.', 'woopress') . '</p>';
                return;
            }
            ...
            if( ! $data['verified'] ) {
               echo  '<p class="error">' . esc_html__('Code is not verified!', 'woopress') . '</p>';
                return;
            }
            ...
            $redirect_url = ( class_exists( 'Redux' ) ) ? admin_url( 'admin.php?page=_options' ) : admin_url( 'themes.php?page=install-required-plugins' ) ;
             ...
             echo   '<p class="updated">' . esc_html__('Theme is activated! You will be redirected in a few seconds', 'woopress') . '
