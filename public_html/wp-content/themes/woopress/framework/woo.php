...
function adjacent_post_link_product( $format, $link, $in_same_cat = false, $excluded_categories = '', $previous = true ) {
    ...
    if ( ! $post ) {
        ...
        if ( empty( $post->post_title ) )
            $title = $previous ? esc_html__( 'Previous Post', 'woopress' ) : esc_html__( 'Next Post', 'woopress' );
        ...
        if ( ! empty( $excluded_categories ) ) {
            if ( ! is_array( $excluded_categories ) ) {
                ...
                if ( strpos( $excluded_categories, ' and ' ) !== false ) {
                    _deprecated_argument( __FUNCTION__, '3.3', sprintf( esc_html__( 'Use commas instead of %s to separate excluded categories.', 'woopress' ), "'and'" ) );
...
function et_product_meta_boxes() {
	add_meta_box( 'woocommerce-product-videos', esc_html__( 'Product Video', 'woopress' ), 'et_woocommerce_product_video_box', 'product', 'side' );
}
...
if(!function_exists('et_woocommerce_product_video_box')) {
	function et_woocommerce_product_video_box() {
		...
		<div id="product_video_container">
			<?php esc_html_e('Upload your Video in 3 formats: MP4, OGG and WEBM', 'woopress') ?>
					...
					if ( $video_attachments )
						foreach ( $video_attachments as $attachment ) {
							echo '<li class="video" data-attachment_id="' . $attachment->id . '">
								...
								<ul class="actions">
									<li><a href="#" class="delete" title="' . esc_html__( 'Delete image', 'woopress' ) . '">' . esc_html__( 'Delete', 'woopress' ) . '</a></li>
								</ul>
							</li>';
						}
				?>
		...
		<p class="add_product_video hide-if-no-js">
			<a href="#"><?php esc_html_e( 'Add product gallery video', 'woopress' ); ?></a>
		</p>
		<p>
			<?php esc_html_e('Or you can use YouTube or Vimeo iframe code', 'woopress'); ?>
		</p>
			...
			jQuery(document).ready(function($){
				...
				jQuery('.add_product_video').on( 'click', 'a', function( event ) {
					...
					product_gallery_frame = wp.media.frames.downloadable_file = wp.media({
						...
						title: '<?php esc_html_e( 'Add Images to Product Gallery', 'woopress' ); ?>',
						button: {
							text: '<?php esc_html_e( 'Add to gallery', 'woopress' ); ?>',
						},
						multiple: true,
						library : { type : 'video'}
					});
							...
							if ( attachment.id ) {
								...
								$product_images.append('\
									<li class="video" data-attachment_id="' + attachment.id + '">\
										Video\
										<ul class="actions">\
											<li><a href="#" class="delete" title="<?php _e( 'Delete video', 'woopress' ); ?>"><?php _e( 'Delete', 'woopress' ); ?></a></li>\
										</ul>\
									</li>');
							}
	...
	function et_product_brand_image() {
        ...
        if(count($terms)>0) {
			...
			<div class="sidebar-widget product-brands">
				<h4 class="widget-title"><span><?php esc_html_e('Product brand', 'woopress') ?></span></h4>
	        	<?php
...
if(!function_exists('et_create_brand_taxonomies')) {
	function et_create_brand_taxonomies() {
		$labels = array(
			'name'              => _x( 'Brands', 'woopress' ),
			'singular_name'     => _x( 'Brand', 'woopress' ),
			'search_items'      => esc_html__( 'Search Brands', 'woopress' ),
			'all_items'         => esc_html__( 'All Brands', 'woopress' ),
			'parent_item'       => esc_html__( 'Parent Brand', 'woopress' ),
			'parent_item_colon' => esc_html__( 'Parent Brand:', 'woopress' ),
			'edit_item'         => esc_html__( 'Edit Brand', 'woopress' ),
			'update_item'       => esc_html__( 'Update Brand', 'woopress' ),
			'add_new_item'      => esc_html__( 'Add New Brand', 'woopress' ),
			'new_item_name'     => esc_html__( 'New Brand Name', 'woopress' ),
			'menu_name'         => esc_html__( 'Brands', 'woopress' ),
		);
...
if(!function_exists('et_brand_fileds')) {
	function et_brand_fileds() {
		...
		<div class="form-field">
			<label><?php esc_html_e( 'Thumbnail', 'woopress' ); ?></label>
			...
			<div style="line-height:60px;">
				...
				<button type="submit" class="upload_image_button button"><?php esc_html_e( 'Upload/Add image', 'woopress' ); ?></button>
				<button type="submit" class="remove_image_button button"><?php esc_html_e( 'Remove image', 'woopress' ); ?></button>
			</div>
			<script type="text/javascript">
				...
				jQuery(document).on( 'click', '.upload_image_button', function( event ){
					...
					file_frame = wp.media.frames.downloadable_file = wp.media({
						title: '<?php _e( 'Choose an image', 'woopress' ); ?>',
						button: {
							text: '<?php _e( 'Use image', 'woopress' ); ?>',
						},
						multiple: false
					});
...
if(!function_exists('et_edit_brand_fields')) {
    function et_edit_brand_fields( $term, $taxonomy ) {
    	...
    	?>
    	<tr class="form-field">
    		<th scope="row" valign="top"><label><?php esc_html_e( 'Thumbnail', 'woopress' ); ?></label></th>
    		<td>
...
if(!function_exists('et_wishlist_btn')) {
    function et_wishlist_btn($label = '') {
        ...
        $html = '';
        if($label == '') {
            $label = esc_html__('Add to Wishlist', 'woopress');
        }
    ...
    function et_size_guide() {
	    if ( etheme_get_custom_field('size_guide_img') ) : ?>
	        ...
	        <div class="size_guide">
	    	 <a rel="<?php echo $lightbox_rel; ?>" href="<?php etheme_custom_field('size_guide_img'); ?>"><?php esc_html_e('SIZING GUIDE', 'woopress'); ?></a>
	        </div>
	    <?php endif;
    }
}
...
global $woocommerce;
        ...
        if ( sizeof( WC()->cart->get_cart() ) > 0 ) {
          ...
          ?>
			<p class="small-h pull-left"><?php echo esc_html__('Cart Subtotal', 'woopress'); ?></p>
...
if ( ! function_exists('et_set_user_visited_product_cookie') ) {
	function et_set_user_visited_product_cookie() {
	    ...
	    if ( is_product() ){
			if ( ! empty( $_COOKIE['woocommerce_recently_viewed'] ) ) {
				$recent_array = explode( '|',$_COOKIE['woocommerce_recently_viewed'] );

				if ( ! in_array( $post->ID, $recent_array ) ) {
					if ( count($recent_array) > 30 ) {
						$recent_products = implode( "|", array_push( array_pop( $recent_array ), $post->ID ) );
						wc_setcookie( 'woocommerce_recently_viewed', $recent_products );
					} else {
						wc_setcookie( 'woocommerce_recently_viewed', $_COOKIE['woocommerce_recently_viewed'].'|'.$post->ID );
					}
				}
			} else {
	...
	add_action( 'wp', 'et_set_user_visited_product_cookie' );
}
