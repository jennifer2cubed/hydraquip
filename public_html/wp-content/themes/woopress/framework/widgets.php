...
class Etheme_Brands_Widget extends WP_Widget {
    ...
    function __construct() {
        $widget_ops = array('classname' => 'etheme_widget_brands', 'description' => esc_html__( "Products Filter by brands", 'woopress') );
        parent::__construct('etheme-brands', '8theme - '.esc_html__('Brands Filter', 'woopress'), $widget_ops);
            ...
            <ul>
                <?php
                    foreach ($terms as $term) {
                        ...
                        ?>
                            <li>
                                <a href="<?php echo get_term_link( $term ); ?>" title="<?php esc_html_e('View all products from ', 'woopress'); echo $term->name; ?>">
...
?>
        <?php etheme_widget_input_text(esc_html__('Title', 'woopress'), $this->get_field_id('title'),$this->get_field_name('title'), $title); ?>
        <?php etheme_widget_input_checkbox(esc_html__('Show hierarchy', 'woopress'), $this->get_field_id('hierarchy'), $this->get_field_name('hierarchy'),checked($hierarchy, true, false), 1); ?>
<?php
    }
}
// **********************************************************************//
...
class Etheme_Search_Widget extends WP_Widget {
    ...
    function __construct() {
        $widget_ops = array('classname' => 'etheme_widget_search', 'description' => esc_html__( "AJAX Search form for Products, Posts, Portfolio and Pages", 'woopress') );
        parent::__construct('etheme-search', '8theme - '.esc_html__('Search Form', 'woopress'), $widget_ops);
...
?>
        ...
        <p><label for="<?php echo $this->get_field_id('post_type'); ?>"><?php esc_html_e('Search type:', 'woopress'); ?></label>
        ...
        <?php etheme_widget_input_checkbox(esc_html__('Display images', 'woopress'), $this->get_field_id('images'), $this->get_field_name('images'),checked($images, true, false), 1); ?>

        <?php etheme_widget_input_text(esc_html__('Number of items', 'woopress'), $this->get_field_id('count'),$this->get_field_name('count'), $count); ?>

        <?php etheme_widget_input_text(esc_html__('Button text', 'woopress'), $this->get_field_id('text'),$this->get_field_name('text'), $text); ?>
...
class Etheme_QRCode_Widget extends WP_Widget {
    ...
    function __construct() {
        $widget_ops = array('classname' => 'etheme_widget_qr_code', 'description' => esc_html__( "You can add a QR code image in sidebar to allow your users get quick access from their devices", 'woopress') );
        parent::__construct('etheme-qr-code', '8theme - '.esc_html__('QR Code', 'woopress'), $widget_ops);
...
?>
        <?php etheme_widget_input_text(esc_html__('Widget title:', 'woopress'), $this->get_field_id('title'),$this->get_field_name('title'), $title); ?>

        <?php etheme_widget_textarea(esc_html__('Information to encode:', 'woopress'), $this->get_field_id('info'),$this->get_field_name('info'), $info); ?>

        <?php etheme_widget_input_text(esc_html__('Image size:', 'woopress'), $this->get_field_id('size'), $this->get_field_name('size'), $size); ?>

        <?php etheme_widget_input_checkbox(esc_html__('Show in lightbox', 'woopress'), $this->get_field_id('lightbox'), $this->get_field_name('lightbox'),checked($lightbox, true, false), 1); ?>

        <?php etheme_widget_input_checkbox(esc_html__('Encode link to the current page', 'woopress'), $this->get_field_id('currlink'), $this->get_field_name('currlink'),checked($currlink, true, false), 1); ?>

        <?php etheme_widget_textarea(esc_html__('Additional information in widget', 'woopress'), $this->get_field_id('text'),$this->get_field_name('text'), $text); ?>
...
class Etheme_StatickBlock_Widget extends WP_Widget {
    ...
    function __construct() {
        $widget_ops = array('classname' => 'etheme_widget_satick_block', 'description' => esc_html__( "Insert static block, that you created", 'woopress') );
        parent::__construct('etheme-static-block', '8theme - '.esc_html__('Statick Block', 'woopress'), $widget_ops);
...
?>
        <p><label for="<?php echo $this->get_field_id('block_id'); ?>"><?php esc_html_e('Block name:', 'woopress'); ?></label>
...
class Etheme_Recent_Posts_Widget extends WP_Widget {
    ...
    function __construct() {
        $widget_ops = array('classname' => 'etheme_widget_recent_entries', 'description' => esc_html__( "The most recent posts on your blog (Etheme Edit)", 'woopress') );
        parent::__construct('etheme-recent-posts', '8theme - '.esc_html__('Recent Posts', 'woopress'), $widget_ops);
                    ...
                    <li>
                        <div class="media">
                            ...
                            <div class="media-body">
                                ...
                                <?php esc_html_e('by', 'woopress') ?> <strong><?php the_author(); ?></strong>
                            </div>
                        </div>
                    </li>
...
?>
        <p><label for="<?php echo $this->get_field_id('title'); ?>"><?php esc_html_e('Title:', 'woopress'); ?></label>
        <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></p>

        <p><label for="<?php echo $this->get_field_id('number'); ?>"><?php esc_html_e('Number of posts to show:', 'woopress'); ?></label>
        <input id="<?php echo $this->get_field_id('number'); ?>" name="<?php echo $this->get_field_name('number'); ?>" type="text" value="<?php echo $number; ?>" size="3" /><br />
        <small><?php esc_html_e('(at most 15)', 'woopress'); ?></small></p>

        <?php etheme_widget_input_checkbox(esc_html__('Enable slider', 'woopress'), $this->get_field_id('slider'), $this->get_field_name('slider'),checked($slider, true, false), 1); ?>
...
class Etheme_Twitter_Widget extends WP_Widget {
    function __construct() {
        $widget_ops = array( 'classname' => 'etheme_twitter', 'description' => esc_html__('Display most recent Twitter feed', 'woopress') );
        $control_ops = array( 'id_base' => 'etheme-twitter' );
        parent::__construct( 'etheme-twitter', '8theme - '.esc_html__('Twitter Feed', 'woopress'), $widget_ops, $control_ops );
    }
    function widget( $args, $instance ) {
        extract( $args );
        ...
        if ( $instance['consumer_key'] && $instance['consumer_secret'] && $instance['user_token'] && $instance['user_secret'] && $instance['usernames'] ) {
            ...
            esc_html_e( 'Not enough information', 'woopress' );
        }
    ...
    function form( $instance ) {
        ...
        etheme_widget_input_text( esc_html__('Title:', 'woopress'), $this->get_field_id( 'title' ), $this->get_field_name( 'title' ), $instance['title'] );
        etheme_widget_input_text( esc_html__('Username:', 'woopress'), $this->get_field_id( 'usernames' ), $this->get_field_name( 'usernames' ), $instance['usernames'] );
        etheme_widget_input_text( esc_html__('Customer Key:', 'woopress'), $this->get_field_id( 'consumer_key' ), $this->get_field_name( 'consumer_key' ), $instance['consumer_key'] );
        etheme_widget_input_text( esc_html__('Customer Secret:', 'woopress'), $this->get_field_id( 'consumer_secret' ), $this->get_field_name( 'consumer_secret' ), $instance['consumer_secret'] );
        etheme_widget_input_text( esc_html__('Access Token:', 'woopress'), $this->get_field_id( 'user_token' ), $this->get_field_name( 'user_token' ), $instance['user_token'] );
        etheme_widget_input_text( esc_html__('Access Token Secret:', 'woopress'), $this->get_field_id( 'user_secret' ), $this->get_field_name( 'user_secret' ), $instance['user_secret'] );
        etheme_widget_input_text( esc_html__('Number of tweets:', 'woopress'), $this->get_field_id( 'limit' ), $this->get_field_name( 'limit' ), $instance['limit'] );
    }
}
...
class Etheme_Flickr_Widget extends WP_Widget {
    ...
    function widget($args, $instance)
    {
        ...
        $title = apply_filters('widget_title', empty( $instance['title'] ) ? esc_html__('Flickr', 'woopress') : $instance['title'], $instance, $this->id_base);
...
class Etheme_Recent_Comments_Widget extends WP_Widget {
    ...
    function __construct() {
        $widget_ops = array('classname' => 'etheme_widget_recent_comments', 'description' => esc_html__( 'The most recent comments (Etheme edit)', 'woopress' ) );
        parent::__construct('etheme-recent-comments', '8theme - '.esc_html__('Recent Comments', 'woopress'), $widget_ops);
        ...
        if ( $comments ) {
            foreach ( (array) $comments as $comment) {
                ...
                $output .=  '<li class="recentcomments">';
                    ...
                    $output .=  ' '.esc_html__('by', 'woopress').' <span class="comment_author">'.get_comment_author_link().'</span>';
                $output .=  '</li>';
            }
        }
        $output .= '</ul>';
...
?>
        ...
        <p><label for="<?php echo $this->get_field_id('number'); ?>"><?php esc_html_e('Number of comments to show:', 'woopress'); ?></label>
...
class null_instagram_widget extends WP_Widget {
    ...
    function __construct() {
		parent::__construct(
			'null-instagram-feed',
			__( 'Instagram', 'woopress' ),
			array(
				'classname' => 'null-instagram-feed',
				'description' => esc_html__( 'Displays your latest Instagram photos', 'woopress' ),
	...
	function form($instance) {
		$instance = wp_parse_args( (array) $instance, array(
            'title' => esc_html__('Instagram', 'woopress'),
            'username' => '',
            'link' => esc_html__('Follow Us', 'woopress'),
            'number' => 9,
            'size' => 'large',
            'target' => '_self',
            'info' => false,
		...
		?>
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php esc_html_e('Title', 'woopress'); ?>: <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></label></p>
		<p><label for="<?php echo $this->get_field_id('username'); ?>"><?php esc_html_e('Username or hashtag', 'woopress'); ?>: <input class="widefat" id="<?php echo $this->get_field_id('username'); ?>" name="<?php echo $this->get_field_name('username'); ?>" type="text" value="<?php echo $username; ?>" /></label></p>
		<p><label for="<?php echo $this->get_field_id('number'); ?>"><?php esc_html_e('Number of photos', 'woopress'); ?>: <input class="widefat" id="<?php echo $this->get_field_id('number'); ?>" name="<?php echo $this->get_field_name('number'); ?>" type="text" value="<?php echo $number; ?>" /></label></p>
		<p><label for="<?php echo $this->get_field_id('size'); ?>"><?php esc_html_e('Photo size', 'woopress'); ?>:</label>
			<select id="<?php echo $this->get_field_id('size'); ?>" name="<?php echo $this->get_field_name('size'); ?>" class="widefat">
				<option value="thumbnail" <?php selected('thumbnail', $size) ?>><?php esc_html_e('Thumbnail', 'woopress'); ?></option>
				<option value="medium" <?php selected('medium', $size) ?>><?php esc_html_e('Medium', 'woopress'); ?></option>
				<option value="large" <?php selected('large', $size) ?>><?php esc_html_e('Large', 'woopress'); ?></option>
			</select>
		</p>
		<p><label for="<?php echo $this->get_field_id('target'); ?>"><?php esc_html_e('Open links in', 'woopress'); ?>:</label>
			<select id="<?php echo $this->get_field_id('target'); ?>" name="<?php echo $this->get_field_name('target'); ?>" class="widefat">
				<option value="_self" <?php selected('_self', $target) ?>><?php esc_html_e('Current window (_self)', 'woopress'); ?></option>
				<option value="_blank" <?php selected('_blank', $target) ?>><?php esc_html_e('New window (_blank)', 'woopress'); ?></option>
			</select>
		</p>
		<p><label for="<?php echo $this->get_field_id('columns'); ?>"><?php esc_html_e('Columns', 'woopress'); ?>:</label>
		...
		<p><label for="<?php echo $this->get_field_id('link'); ?>"><?php esc_html_e('Link text', 'woopress'); ?>: <input class="widefat" id="<?php echo $this->get_field_id('link'); ?>" name="<?php echo $this->get_field_name('link'); ?>" type="text" value="<?php echo $link; ?>" /></label></p>
		<p>
			<input type="checkbox" <?php checked( true, $info, true); ?> id="<?php echo $this->get_field_id('info'); ?>" name="<?php echo $this->get_field_name('info'); ?>">
			<label for="<?php echo $this->get_field_id('info'); ?>"><?php esc_html_e('Additional information', 'woopress'); ?></label>
		</p>
		<p>
			...
			<label for="<?php echo $this->get_field_id('slider'); ?>"><?php esc_html_e('Carousel', 'woopress'); ?></label>
		</p>
		<p>
			...
			<label for="<?php echo $this->get_field_id('spacing'); ?>"><?php esc_html_e('Without spacing', 'woopress'); ?></label>
		</p>
		<?php
    	...
    	function scrape_instagram( $username ) {
    		...
    		if ( false === ( $instagram = get_transient( 'insta-a10-' . $transient_prefix . '-' . sanitize_title_with_dashes( $username ) ) ) ) {
    			...
    			if ( is_wp_error( $remote ) ) {
    				return new WP_Error( 'site_down', esc_html__( 'Unable to communicate with Instagram.', 'woopress' ) );
    			}

    			if ( 200 !== wp_remote_retrieve_response_code( $remote ) ) {
    				return new WP_Error( 'invalid_response', esc_html__( 'Instagram did not return a 200.', 'woopress' ) );
    			}
    			...
    			if ( ! $insta_array ) {
    				return new WP_Error( 'bad_json', esc_html__( 'Instagram has returned invalid data.', 'woopress' ) );
    			}
                ...
                if ( isset( $insta_array['entry_data']['ProfilePage'][0]['graphql']['user']['edge_owner_to_timeline_media']['edges'] ) ) {
    				...
    				return new WP_Error( 'bad_json_2', esc_html__( 'Instagram has returned invalid data.', 'woopress' ) );
    			}

    			if ( ! is_array( $images ) ) {
    				return new WP_Error( 'bad_array', esc_html__( 'Instagram has returned invalid data.', 'woopress' ) );
    			}
                ...
                foreach ( $images as $image ) {
                    ...
                    $caption = __( 'Instagram Image', 'woopress' );
    		...
    		if ( ! empty( $instagram ) ) {
    			...
    			return new WP_Error( 'no_images', esc_html__( 'Instagram did not return any images.', 'woopress' ) );
...
class Etheme_Socials_Widget extends WP_Widget {
    ...
    function __construct() {
        $widget_ops = array('classname' => 'etheme_widget_socials', 'description' => esc_html__( "Social links widget", 'woopress') );
        parent::__construct('etheme-socials', '8theme - '.esc_html__('Social links', 'woopress'), $widget_ops);
    ...
    function form( $instance ) {
        ...
        etheme_widget_input_text(esc_html__('Title', 'woopress'), $this->get_field_id('title'),$this->get_field_name('title'), $title);
        etheme_widget_input_dropdown(esc_html__('Size', 'woopress'), $this->get_field_id('size'),$this->get_field_name('size'), $size, array(
            'small' => 'Small',
            'normal' => 'Normal',
            'large' => 'Large',
        ));

        etheme_widget_input_text(esc_html__('Facebook link', 'woopress'), $this->get_field_id('facebook'),$this->get_field_name('facebook'), $facebook);
        etheme_widget_input_text(esc_html__('Twitter link', 'woopress'), $this->get_field_id('twitter'),$this->get_field_name('twitter'), $twitter);
        etheme_widget_input_text(esc_html__('Instagram link', 'woopress'), $this->get_field_id('instagram'),$this->get_field_name('instagram'), $instagram);
        etheme_widget_input_text(esc_html__('Google + link', 'woopress'), $this->get_field_id('google'),$this->get_field_name('google'), $google);
        etheme_widget_input_text(esc_html__('Pinterest link', 'woopress'), $this->get_field_id('pinterest'),$this->get_field_name('pinterest'), $pinterest);
        etheme_widget_input_text(esc_html__('LinkedIn link', 'woopress'), $this->get_field_id('linkedin'),$this->get_field_name('linkedin'), $linkedin);
        etheme_widget_input_text(esc_html__('Tumblr link', 'woopress'), $this->get_field_id('tumblr'),$this->get_field_name('tumblr'), $tumblr);
        etheme_widget_input_text(esc_html__('YouTube link', 'woopress'), $this->get_field_id('youtube'),$this->get_field_name('youtube'), $youtube);
        etheme_widget_input_text(esc_html__('Vimeo link', 'woopress'), $this->get_field_id('vimeo'),$this->get_field_name('vimeo'), $vimeo);
        etheme_widget_input_text(esc_html__('RSS link', 'woopress'), $this->get_field_id('rss'),$this->get_field_name('rss'), $rss);
        etheme_widget_input_checkbox(esc_html__('Colorfull icons', 'woopress'), $this->get_field_id('colorfull'),$this->get_field_name('colorfull'), checked( 1, $colorfull, false ), 1);

        etheme_widget_input_dropdown(esc_html__('Link Target', 'woopress'), $this->get_field_id('target'),$this->get_field_name('target'), $target, array(
