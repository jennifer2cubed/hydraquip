...
if(!function_exists('et_add_body_classes')) {
    function et_add_body_classes($classes) {
        ...
        if(etheme_get_option('fixed_nav_sw') == 'on') $classes[] = 'fixNav-enabled ';
        ...
        if(!class_exists('Woocommerce') || etheme_get_option('just_catalog') || etheme_get_option('cart_widget_sw') == 'off') $classes[] = 'top-cart-disabled ';
            ...
            <style type="text/css">
                ...
                <?php et_load_bg_option( etheme_get_option('top_bar_bg'), 'div[class*="header-type-"] .top-bar,div[class*="header-type-"].slider-overlap .top-bar,div[class*="header-type-"].slider-overlap .top-bar > .container,div[class*="header-type-"] .top-bar > .container' ); ?>
                 ...
                 <?php
                    ...
                    $menu_states = etheme_get_option( 'menufont_link_color' );
                    $menu_l2_states = etheme_get_option( 'menufont_2_link_color' );
                    $menu_l3_states = etheme_get_option( 'menufont_3_link_color' );
                  ?>
                 ...
                 <?php if ( ! empty( $menu_states['hover'] ) ) echo jsString($selectors['menu_font_hover']); ?> { color: <?php echo $menu_states['hover']; ?> !important; }
                 <?php if ( ! empty( $menu_states['active'] ) ) echo jsString($selectors['menu_font_active']); ?> { color: <?php echo $menu_states['active']; ?> !important; }

                 <?php if ( ! empty( $menu_l3_states['hover'] ) ) echo jsString($selectors['menu_font_l3_hover']); ?> { color: <?php echo $menu_l3_states['hover']; ?> !important; }
                 <?php if ( ! empty( $menu_l3_states['active'] ) ) echo jsString($selectors['menu_font_l3_active']); ?> { color: <?php echo $menu_l3_states['active']; ?> !important; }

                 <?php if ( ! empty( $menu_l2_states['hover'] ) ) echo jsString($selectors['menu_font_l2_hover']); ?> { color: <?php echo $menu_l2_states['hover']; ?> !important; }
                 <?php if ( ! empty( $menu_l2_states['active'] ) ) echo jsString($selectors['menu_font_l2_active']); ?> { color: <?php echo $menu_l2_states['active']; ?> !important; }
            </style>
            <?php
...
if(!function_exists('et_wp_title')) {
    ...
    function et_wp_title($title) {
        ...
        if ( $paged >= 2 || $page >= 2 )
            $title .= ' | ' . esc_html__( 'Page ', 'woopress' ) . max( $paged, $page ) ;
                    ...
                    <?php else : ?>
                        ...
                        <?php if( get_option('users_can_register') && ( ! empty( $reg_id ) || ( isset( $args['register'] ) && $args['register'] == true ) ) ): ?>
                            <li class="register-link">
                                <a href="<?php echo $reg_url; ?>"><?php esc_html_e( 'Register', 'woopress' ); ?></a>
                                <?php if($popups) : ?>
...
if(!function_exists('et_send_msg_action')) {
    function et_send_msg_action() {
        ...
        if(isset($_GET['contact-submit'])) {
            ...
            if( ! $captcha_instance->check( $_GET['captcha-prefix'], $_GET['captcha-word'] ) ) $return['msg'][] = esc_html__( 'The security code you entered did not match. Please try again.', 'woopress' );
...
if(!function_exists('et_register_action')) {
    function et_register_action() {
        ...
        if( ! empty( $_POST['input_data'] ) ){
            ...
            if( ! $captcha_instance->check( $captcha_prefix, $captcha_word ) ) $return['msg'][] = esc_html__('The security code you entered did not match. Please try again.', 'woopress');
                ...
                <div class="media">
                    ...
                    <?php if ( $rating && get_option( 'woocommerce_enable_review_rating' ) == 'yes' ) : ?>
                        ...
                        <div class="star-rating" title="<?php echo sprintf( esc_html__( 'Rated %d out of 5', 'woopress' ), $rating ) ?>">
                            <span style="width:<?php echo ( $rating / 5 ) * 100; ?>%"><strong><?php echo $rating; ?></strong> <?php esc_html_e( 'out of 5', 'woopress' ); ?></span>
                        </div>
...
if(!function_exists('etheme_custom_comment_form')) {
    function etheme_custom_comment_form($defaults) {
        ...
        $defaults['comment_field'] = '<div class="form-group"><label for="comment" class="control-label">'.__('Comment', 'woopress').'</label><textarea class="form-control required-field"  id="comment" name="comment" cols="45" rows="12" aria-required="true"></textarea></div>';
    ...
    function etheme_teaser($atts, $content = null) {
        ...
        while ( $my_query->have_posts() ) {
            ...
            if ( $grid_link != 'link_no' ) {
                $link = '<a class="more-link" href="'. get_permalink($post_id) .'"'.$link_target.' title="'. esc_attr__( 'Permalink to ', 'js_composer' ) . the_title_attribute( 'echo=0' ) .'">'. esc_html__("Read more", "js_composer") .'</a>';
            }
            ...
            if ( $grid_link != 'link_no' ) {
                ...
                if ( $grid_link == 'link_post' ) {
                    $link_image_start = '<a class="link_image" href="'.get_permalink($post_id).'"'.$link_target.' title="'. esc_attr__( 'Permalink to ', 'js_composer' ) . the_title_attribute( 'echo=0' ).'">';
                    $link_title_start = '<a class="link_title" href="'.get_permalink($post_id).'"'.$link_target.' title="'.esc_attr__( 'Permalink to ', 'js_composer' ) . the_title_attribute( 'echo=0' ) .'">';
                }
        ...
        if( $grid_template == 'filtered_grid' && $teasers && !empty($teaser_categories)) {
            ...
            $categories_list_output .= '<li class="active"><a href="#" data-filter="*" class="button active">' . esc_html__('All', 'js_composer') . '</a></li>';
...
if(!function_exists('etheme_breadcrumbs')) {
    function etheme_breadcrumbs() {
      ...
      $blogPage = esc_html__('Blog', 'woopress');
      ...
      if (is_front_page()) {
        ...
        if ( is_category() ) {
          ...
          echo $before .  esc_html__( 'Archive by category "', 'woopress' ) . single_cat_title('', false) . '"' . $after;

        } elseif ( is_search() ) {
          echo $before . esc_html__( 'Search results for "', 'woopress' ) . get_search_query() . '"' . $after;
          ...
          echo $before .  esc_html__( 'Posts tagged "', 'woopress' ) . single_tag_title('', false) . '"' . $after;

        } elseif ( is_author() ) {
           global $author;
          $userdata = get_userdata($author);
          echo $before .  esc_html__( 'Articles posted by ', 'woopress' ) . $userdata->display_name . $after;

        } elseif ( is_404() ) {
          echo $before . esc_html__( 'Error 404', 'woopress' ) . $after;
        }else{
...
function etheme_print_tweet_error($errorArray) {
    return '<p class="eth-error">' . esc_html__('Error: ', 'woopress' ) . $errorArray['message'] . esc_html__('in ', 'woopress' ) . $errorArray['file'] . esc_html__( 'on line ', 'woopress' ) . $errorArray['line'] . '</p>';
...
if( ! function_exists( 'etheme_activation_bar_out' ) ) :
    function etheme_activation_bar_out() {
        ?>
            <div class="etheme-activation-bar"><?php esc_html_e('Important Note: You need to ', 'woopress' ); ?><a href="<?php echo admin_url( 'admin.php?page=etheme_activation_page' ); ?>"><?php esc_html_e('activate Woopress template', 'woopress' ); ?> </a> <?php esc_html_e( 'with your purchase code to continue working.', 'woopress' ); ?></div>
        <?php
    }
endif;
...
if(!function_exists('et_register_static_blocks')) {
    function et_register_static_blocks() {
            $labels = array(
                'name' => _x( 'Static Blocks', 'post type general name', 'woopress' ),
                'singular_name' => _x( 'Block', 'post type singular name', 'woopress' ),
                'add_new' => _x( 'Add New', 'static block', 'woopress' ),
                'add_new_item' => sprintf( esc_html__( 'Add New %s', 'woopress' ), esc_html__( 'Static Blocks', 'woopress' ) ),
                'edit_item' => sprintf( esc_html__( 'Edit %s', 'woopress' ), esc_html__( 'Static Blocks', 'woopress' ) ),
                'new_item' => sprintf( esc_html__( 'New %s', 'woopress' ), esc_html__( 'Static Blocks', 'woopress' ) ),
                'all_items' => sprintf( esc_html__( 'All %s', 'woopress' ), esc_html__( 'Static Blocks', 'woopress' ) ),
                'view_item' => sprintf( esc_html__( 'View %s', 'woopress' ), esc_html__( 'Static Blocks', 'woopress' ) ),
                'search_items' => sprintf( esc_html__( 'Search %a', 'woopress' ), esc_html__( 'Static Blocks', 'woopress' ) ),
                'not_found' =>  sprintf( esc_html__( 'No %s Found', 'woopress' ), esc_html__( 'Static Blocks', 'woopress' ) ),
                'not_found_in_trash' => sprintf( esc_html__( 'No %s Found In Trash', 'woopress' ), esc_html__( 'Static Blocks', 'woopress' ) ),
                'parent_item_colon' => '',
                'menu_name' => esc_html__( 'Static Blocks', 'woopress' )
...
if( ! function_exists( 'etheme_deactivate_theme' ) ) {
    function etheme_deactivate_theme() {
        ...
        // }
        // else {
        //     update_option( 'royal_api_key', 0 );
        //     update_option( 'royal_purchase_code', 0 );
        // }
