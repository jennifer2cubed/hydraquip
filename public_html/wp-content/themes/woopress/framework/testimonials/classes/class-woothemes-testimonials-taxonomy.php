...
class Woothemes_Testimonials_Taxonomy {
	...
	public function __construct ( $token = 'testimonial-category', $singular = '', $plural = '', $args = array() ) {
		...
		if ( '' == $this->singular ) $this->singular = esc_html__( 'Category', 'woopress' );
		if ( '' == $this->plural ) $this->plural = esc_html__( 'Categories', 'woopress' );
	...
	private function _get_default_labels () {
		return array(
			    'name'                => sprintf( _x( '%s', 'taxonomy general name', 'woopress' ), $this->plural ),
			    'singular_name'       => sprintf( _x( '%s', 'taxonomy singular name', 'woopress' ), $this->singular ),
			    'search_items'        => sprintf( esc_html__( 'Search %s', 'woopress' ), $this->plural ),
			    'all_items'           => sprintf( esc_html__( 'All %s', 'woopress' ), $this->plural ),
			    'parent_item'         => sprintf( esc_html__( 'Parent %s', 'woopress' ), $this->singular ),
			    'parent_item_colon'   => sprintf( esc_html__( 'Parent %s:', 'woopress' ), $this->singular ),
			    'edit_item'           => sprintf( esc_html__( 'Edit %s', 'woopress' ), $this->singular ),
			    'update_item'         => sprintf( esc_html__( 'Update %s', 'woopress' ), $this->singular ),
			    'add_new_item'        => sprintf( esc_html__( 'Add New %s', 'woopress' ), $this->singular ),
			    'new_item_name'       => sprintf( esc_html__( 'New %s Name', 'woopress' ), $this->singular ),
			    'menu_name'           => sprintf( esc_html__( '%s', 'woopress' ), $this->plural )
			  );
